...
I <- function(j){j=j+1L} # example: n <- 99L; n%<>%I; n; # 100
# `%I%`<-function(x,y){x%<>% `+`(y);return(x)}; # eg. a=3;a%I%1;a;#4
...
source("List2Rows.R") # assumes getwd() for  is "someting/R"
...
kable(jobCode,caption="Job Codes");
```
...
## Simplify Choices re IYS Topics
If a person expressed interest in collaboration on an IYS topic/theme, that is the basis for assigning that topic as an IdeaTag. The choices in *survey1* are converted to binary: interested or not, 1 or zero. The category *other* for each theme is, when proferred, a list of new ideas and is dealt with separately.  The result is 0 or 1 and replaces the original choices in *survey1*. 
...
x <- survey1[ , topic.col] # not 163, this is 367 by 37 with about 204 not repondents.
x[is.na(x) | x == 1] <- 0 # missing or choice is "no, not applicable"
x[x > 1] <- 1 # choice 2 is meh but choice3, 4, or 5 means definitely interested.
survey1[ ,topic.col] <- x # replace this into the data.frame
```

## Extract Names for IYS Theme and Topic  
Lists of theme and topic names, and a factor *fctr* that relates the 37 topics to the 6 themes. Topics and themes will be *IdeaTags*, with topics nested within themes, that link people to other people. Not selecting a choice for a topic, or selecting choice 1 (*No, not applicable*) interest) was taken as less than definitely interested and that tag was not applied. Note that choice 2 (*Yes, but unlikely at present*) indicates weaker interest in collaboration on a topic than choices 3 to 5.
...
The survey data has cells that are lists of attributes for a Person, edited by hand to QCSV. E.g. *webPage* can be a comma-separated list:  
...
So a *Person* can have multiple of *webPage*, and a *webPage* canbe associated with more than one *Person*.
...
PersonHasIdeaTag <- data.frame(Person.name = NA,IdeaTag.name = NA);
jPHI <- 0; 
for (j in 1:dim(survey1)[1]){                # 367
    for (k in 1:length(topic.col)){          #  37
        if(survey1[j, topic.col[k] ] == 1){
            jPHI <- jPHI+1;                  # 3551 
...
This created 367 Person and `r jPHI` of PersonHasIdeaTag. 
...
## Activity, PersonHasActivity, ActivityHasPerson, ActivityHasLocation, IdeaTagA, ActivityHasIdeaTag, ActivityHasWebPage
...
ActivityHasWebPage  <- data.frame(Activity.name=NA,IdeaTag.name=NA);
...
## Place, 
I wanted to eliminate Person has Place in favour of Person has Activity hsa Place, but mostly we
```{r}
...
```{r write.csv}
write.csv(Activity,file="../output/Activity.csv");
write.csv(ActivityHasIdeaTag,file="../output/ActivityHasIdeaTag.csv");
write.csv(ActivityHasLocation,file="../output/ActivityHasLocation.csv");
write.csv(ActivityHasPerson,file="../output/ActivityHasPerson.csv");
write.csv(ActivityHasWebPage,file="../output/ActivityHasWebPage.csv");
write.csv(IdeaTag,file="../output/IdeaTag.csv");
write.csv(IdeaTagA,file="../output/IdeaTagA.csv");
write.csv(OrgHasOrg,file="../output/OrgHasOrg.csv");
write.csv(Person,file="../output/Person.csv");
write.csv(PersonHasContactService,file="../output/PersonHasContactService.csv");
write.csv(PersonHasActivity,file="../output/PersonHasActivity.csv");
write.csv(PersonHasIdeaTag,file="../output/PersonHasIdeaTag.csv");
write.csv(PersonHasIdeaTagB,file="../output/PersonHasIdeaTagB.csv");
write.csv(PersonHasOrg,file="../output/PersonHasOrg.csv");
write.csv(PersonHasPlace,file="../output/PersonHasPlace.csv");
write.csv(PlaceHasAddress,file="../output/PlaceHasAddress.csv"); 
